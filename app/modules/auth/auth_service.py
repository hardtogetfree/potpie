    ...
    async def check_auth(
        ...
        if os.getenv("isDevelopmentMode") == "enabled" and credential is None:
            ...
            try:
                ...
                # If email missing from token, get from database
                if not decoded_token.get('email'):
                    logging.info("DEBUG: Email missing from token, fetching from database")
                    ...
                    try:
                        ...
                        user = user_service.get_user_by_uid(decoded_token['user_id'])
                        if user and user.email:
                            decoded_token['email'] = user.email
                            logging.info(f"DEBUG: Retrieved email from database: {user.email}")
                        else:
                            logging.warning(f"DEBUG: No email found for user {decoded_token['user_id']}")
                    finally:
                        db.close()
